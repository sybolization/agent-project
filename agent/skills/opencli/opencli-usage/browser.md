...
当搜索或查询命令返回包含URL的结果时，必须使用完整URL，包括所有查询参数。

**使用方法**：
- 使用命令返回的完整URL（包含所有查询参数如 xsec_token、token 等）
- 使用 `opencli browser open "完整URL"` 打开对应页面
- **不要**截断URL中的查询参数，缺少参数会导致页面无法访问
...
# 错误 - 截断查询参数
opencli browser open "https://www.xiaohongshu.com/search_result/xxx"
```
...
> **获取笔记内容**：由于 `note` 命令已被屏蔽，请使用 `opencli browser open <url>` 直接打开笔记页面查看内容。
