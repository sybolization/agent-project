...
description: |
  Browser automation for navigating, clicking, typing, and extracting data from websites.
  USE WHEN user wants to open a webpage, click elements, type text, scroll, or interact with page content.
  USE WHEN user wants to navigate to a specific URL.
  USE WHEN user wants to click on elements (buttons, links, videos).
  USE WHEN user wants to extract data from a webpage.
...
**URL使用规则**：当其他命令返回包含URL的结果时，必须使用完整URL（包含所有查询参数）。使用 `opencli browser open "完整URL"` 打开对应页面。不要截断URL中的查询参数。

**Always chain when possible** — fewer tool calls = faster completion:
```bash
# GOOD: open + inspect in one call (saves 1 round trip)
opencli browser open [ref:1] && opencli browser state
...
1. **Navigate**: `opencli browser open "完整URL"` (使用命令返回的完整URL)
...
```bash
opencli browser open "完整URL"           # 使用命令返回的完整URL（包含查询参数）
opencli browser open <url>              # 仅当没有命令返回的URL时使用
...
opencli browser wait text "Success"               # Wait for text
...
```bash
opencli browser init hn/top              # Generate adapter scaffold
...
| Element not found | `opencli browser scroll down` then `opencli browser state` |
| Stale indices after page change | Run `opencli browser state` again to get fresh indices |

## Windows-Specific Notes

### Command Chaining
On Windows, `&&` does NOT work reliably in subprocess execution. Execute commands separately:

```bash
# BAD: Chaining with && (may fail on Windows)
...
### eval Command
When using `eval` on Windows, be careful with argument parsing. PowerShell may split arguments incorrectly:

```bash
# BAD: Complex JS with spaces (may fail on Windows)
opencli browser eval "JSON.stringify([...document.querySelectorAll('h2')].map(e => e.textContent))"

# GOOD: Simple expressions
opencli browser eval "document.title"

# GOOD: Use get text instead for data extraction
opencli browser get text <index>

# GOOD: Use get html for larger content
opencli browser get html --selector "h2"
```

### Alternative Data Extraction
Instead of complex `eval` commands, prefer these alternatives on Windows:

1. **`get text <index>`** — Get text content of a specific element
2. **`get html --selector "css"`** — Get HTML of elements matching selector
3. **`get title` / `get url`** — Get page metadata
4. **`state`** — Get structured DOM with element indices
